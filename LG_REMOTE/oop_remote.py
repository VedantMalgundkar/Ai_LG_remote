        ...
        while True:
            ...
            if res.multi_hand_landmarks is not None:
                for hand_landmarks, handedness in zip(res.multi_hand_landmarks, res.multi_handedness):
                    ...
                    brect = self.calc_bounding_rect(debug_img, hand_landmarks)

                    # landmark_list calculation
                    landmark_list = self.calc_landmark_list(debug_img, hand_landmarks)
                    ...
                    debug_img = self.draw_bounding_rect(use_brect, debug_img, brect)
                    debug_img = self.draw_landmarks(debug_img, landmark_list)

                    if hand_sign_label is not None:
                        debug_img = self.draw_info_text(debug_img, brect, handedness,
                                                        handsign_classifier_labels[hand_sign_label])

            debug_img = self.draw_info(debug_img, fps, mode, number)
